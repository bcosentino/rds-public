...
\begin{table}[!h]
\centering
\caption{\label{tab:c02c01}Classification of research questions by primary goal}
\centering
\begin{tabular}[t]{p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}p{0.3\linewidth}}
\toprule
...
\FunctionTok{ggplot}\NormalTok{(d, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ distance)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{binwidth =} \DecValTok{300}\NormalTok{, }\AttributeTok{boundary =} \DecValTok{0}\NormalTok{, }
                 \AttributeTok{color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"lightblue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{facet\_grid}\NormalTok{(origin }\SpecialCharTok{\textasciitilde{}}\NormalTok{ .) }\SpecialCharTok{+}
...
{\centering \includegraphics{ch07_estimation_files/figure-latex/c0c15-1} 

}

\caption{Likelihood distribution showing the probability of the observed data (1 out of 15 positive tests) conditional on each value of the prevalence of infection.}\label{fig:c0c15}
\end{figure}
...
\section{Estimation for a continuous random variable}\label{estimation-for-a-continuous-random-variable}
...
use are included in an R packages called \emph{NHANES}, which stands for the
...
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{5}\NormalTok{, }\DecValTok{20}\NormalTok{, }\AttributeTok{length.out =} \DecValTok{2000}\NormalTok{)}
\NormalTok{dens }\OtherTok{\textless{}{-}} \FunctionTok{ifelse}\NormalTok{(x }\SpecialCharTok{\textless{}}\NormalTok{ a }\SpecialCharTok{|}\NormalTok{ x }\SpecialCharTok{\textgreater{}}\NormalTok{ b, }\DecValTok{0}\NormalTok{, }\FunctionTok{dunif}\NormalTok{(x, }\AttributeTok{min =}\NormalTok{ a, }\AttributeTok{max =}\NormalTok{ b))}
\NormalTok{df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{dens =}\NormalTok{ dens)}

\FunctionTok{ggplot}\NormalTok{(df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ dens)) }\SpecialCharTok{+}
  \FunctionTok{geom\_area}\NormalTok{(}\AttributeTok{fill =} \StringTok{"skyblue"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_step}\NormalTok{(}\AttributeTok{direction =} \StringTok{"hv"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Parameter value"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Probability Density"}\NormalTok{, }\AttributeTok{title =} 
         \StringTok{"Prior distribution for the standard deviation of heart rate"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{coord\_cartesian}\NormalTok{(}\AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{2}\NormalTok{, }\DecValTok{17}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics{ch07_estimation_files/figure-latex/c07c40-1} \end{center}
...
With our statistical model in defined, let's proceed with estimation in \texttt{brms}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#model formula: heart rate is normally distributed}
...
\NormalTok{m1.prior }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}
  \FunctionTok{prior}\NormalTok{(}\FunctionTok{normal}\NormalTok{(}\DecValTok{75}\NormalTok{, }\DecValTok{10}\NormalTok{), }\AttributeTok{class =} \StringTok{"Intercept"}\NormalTok{),}
  \FunctionTok{prior}\NormalTok{(}\FunctionTok{uniform}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{20}\NormalTok{), }\AttributeTok{class =} \StringTok{"sigma"}\NormalTok{, }\AttributeTok{lb =} \DecValTok{0}\NormalTok{, }\AttributeTok{ub =} \DecValTok{20}\NormalTok{)}
\NormalTok{)}
...
Let's walk through this. We've defined the likelihood with the \texttt{bf} function, where the formula \texttt{Pulse\ \textasciitilde{}\ 1} tells \texttt{brms} that we want to fit a model with only an intercept, which is just a fancy way of saying we want the mean of the normal distribution that we specified with \texttt{family\ =\ gaussian} (recall ``Gaussian'' is another name for the normal distribution). We then define our priors. In \texttt{brms}, each parameter getting a prior needs a distribution for the prior. You can see we specify \texttt{normal(75,\ 10)} for the mean and \texttt{uniform(0,\ 20)} for the standard deviation in this case. The \texttt{class} argument is required to indicate the type of variable, here being an \texttt{Intercept} for the mean and \texttt{sigma} for the standard deviation.

After fitting the model, we print the model output (\texttt{print(m1)}) to see same basic numerical summaries of the posterior distributions for the parameters we estimated. Our estimate for the mean heart rate is listed under ``Regression Coefficients'' as the ``Intercept''. Remember that our estimate is an entire probability distribution, and \texttt{brms} defaults to showing you the mean of the posterior as the ``Estimate''. Thus we see that them ean of the posterior distribution for resting heart rate is 72.57 bpm. The ``Est.Error'' represents the standard deviation of the posterior distribution, and we also see limits of a 95\% credible interval (CI). Based on the analysis, we can say there's a 95\% probability that the mean resting heart rate is between 72.29 bpm and 72.85 bpm.
...
It's worth noting here that although \texttt{brms} shows 95\% credible intervals by default, there's nothing special about the 95\% level. Indeed, a confidence level of 94\% or 96\% would probably tell the same story about uncertainty. The use of 95\% is largely historical and related to the use of particular thresholds of ``statistical significance'' with frequentist approaches to estimation. But there's no reason to obsess about using a 95\% credible interval. One just needs to be aware that credible intervals will widen as you increase the probability level, and they'll decrease as you narrow the probability level. And it's straightforward to ask \texttt{brms} to show credible intervals at different probability levels using the \texttt{prob} argument in the \texttt{summary} function. Here you can see a summary of the posterior distribution with a 92\% credible interval:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(m1, }\AttributeTok{prob=}\FloatTok{0.92}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{ch07_estimation_files/figure-latex/c07c44-1} \end{center}

We could also compute other summary statistics that are automatically reported by \texttt{brms} in the summary:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#probability that the mean height is \textgreater{}75 bpm}
\FunctionTok{mean}\NormalTok{(m1.post}\SpecialCharTok{$}\NormalTok{b\_Intercept}\SpecialCharTok{\textgreater{}}\DecValTok{75}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#80\% credible interval}
\FunctionTok{quantile}\NormalTok{(m1.post}\SpecialCharTok{$}\NormalTok{b\_Intercept, }\AttributeTok{probs=}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\FloatTok{0.1}\NormalTok{, }\FloatTok{0.9}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
The posterior distributions estimated from our model are about the population mean and standard deviation. But what if we were interested in what the model implied about the \emph{individual} heart rates? For example, how likely is it, based on our model, that an individual has tachycardia, defined as a resting heart rate over 100 bpm? We can interrogate questions like this by combining the information from our posterior distributions on the mean and standard deviation. As we can see in the header of the posterior samples above (\texttt{head(m1.post)}), every sample of the posterior ditribution includes a value of the mean (\texttt{b\_Intercept}) and standard deviation (\texttt{sigma}) of resting heart rates. These values vary from sample to sample, collectively describing the uncertainty about the parametrs. We can leverage that variation to derive a posterior distribution for the individual heart rates implied by the mean and standard deviation. One simple way to do this is by simulation, drawing a random value of an individual's heart rate from the normal distribution implied by each sample in the posterior:
...
\begin{center}\includegraphics{ch07_estimation_files/figure-latex/c07c46-1} \end{center}

The resulting distribution is called a \textbf{joint posterior distribution}, and we could summarize it just as we would one of the individual posterior distribution, the main difference being that here we are talking about individual heart rates. For example, we can use the joint posterior to compute the probability that an individual in this population has tachycardia:
...
\section{Prior distribution choices}\label{prior-distribution-choices}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c22-1} 

}

\caption{TODO: caption.}\label{fig:c09c22}
\end{figure}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c26-1} 

}

\caption{TODO: caption.}\label{fig:c09c26}
\end{figure}
...
\begin{center}\includegraphics{ch09_linearModels_files/figure-latex/c09c28-1} \end{center}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c29-1} 

}

\caption{TODO: caption.}\label{fig:c09c29}
\end{figure}
...
\begin{center}\includegraphics{ch09_linearModels_files/figure-latex/c09c31-1} \end{center}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c32-1} 

}

\caption{TODO: caption.}\label{fig:c09c32}
\end{figure}
...
\begin{center}\includegraphics{ch09_linearModels_files/figure-latex/c09c34-1} \end{center}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c35-1} 

}

\caption{TODO: caption.}\label{fig:c09c35}
\end{figure}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c38-1} 

}

\caption{TODO: caption.}\label{fig:c09c38}
\end{figure}
...
{\centering \includegraphics{ch09_linearModels_files/figure-latex/c09c40-1} 

}

\caption{TODO: caption.}\label{fig:c09c40}
\end{figure}
